...
If a card has a rule that places it back in the hand of the player who played it, and a rule which makes it that player's turn again, then the AI can enter a loop. Two questions arise from this: can the game ever lock up, meaning that a player in that situation cannot move the game out of that state, and depending on that answer how should we either (in the no case,) make the AI smarter or, adjust the list of possible rules to remove this possibility.

First we need to know whether the game can lock up. Let's try looking at extremes to find a place where it is clear the game has locked up. If all the other cards are in other player's hands then the player with the one card can currently "draw" no cards and advance the turn. There is currently no way for drawing a card, (or no card) to do anything different than usual but that may change in the future. Another scenario: all the cards have the same-player-turn, return-self-to-player-hand property. Initially all players have 5 cards. The players could darw the deck dry but still no one would have any way to change anything after that, (assuming no other rules, which is of course a possibility). So the game can be locked up by this rule combination!

So now we need to decide how to adjust the available rules to prevent this possibility. There are two rules that combine to create the issue, so given we want as many interesting combinations of rules as possible, we have two options: remove the "same-turn" aspect or remove the "return-to-player's hand" aspect. 

Removing the "same player turn" implies removing all the turn order rules since 4 next or previous shifts is the same as going back to the same player, and 2 across shifts are as well. While removing those seems like a clean way to solve the problem (there's no reason to wonder why something was included when something else wasn't,) it removes a lot of possible rule sets with potentially interesting decisions. For instance, a player may have to decide between whether they should prevent a player from having a turn or play a card which changes the suit to something that most of the rest of their hand is.

Removing the "return to the player's hand" aspect, at first glance, seems like it can be done by removing the discard to self rule. But can other rules compose to produce the same effect, like the turn changing rules do? If the deck and discard piles are empty, and there is a card with a rule that says to put the card into the deck, then put a card from the deck into your hand, then that is the same as putting the card into your hand. We could also remove the deck to your hand rule to prevent this possibility. You might think since the other player's hands cannot be empty then we can't end up in a loop passing the card into another players hand and then to the current player's hand. But if the rule states to take the nth card from that player's hand and the player happens to have the exactly right number of cards then we still can! So far all of these loops have needed the turn to be immediately returned to the same player in to order to actually loop, suggesting we should just remove those rules. But can we create a loop without the turn shift rules? Let's say that it's the start of the game and each player has 5 cards. If all the cards have a rule that says move a card from the discard pile to say the next player's hand then move the 6th card from that player's hand to the player who played the card, then, assuming that the players all play a card then the game would be returned to the same state as before, after every player has taken a turn. But player's can draw a card instead! Even if every player has 13 cards, (meaning the deck and discard piles are empty,) then currently players can still "draw" no card at all, effectively skipping their turn. But that doesn't change the state either! So we can still have a loop even without the turn shift rules.

Given that the card movements are enough to cause loops, it appears we should change the card movements. But since we want the most interesting combinations to be possible, what is the fewest number of rules we can remove? Are there enough left after that to be worth including? If not, would it be better to simply accept the amount of possible loop states that exist? Can we check for those specifically?

____

I have made a visualization of the card movement graph and it is simpler than I thought. It is essentially a 6-clique. Given that fact, the way to prevent cycles involving the current player which removes the fewest edges is to remove *all* edges that put cards in the current player's hand. 

Also of note, I have realized that the above argument involving a scenario where all the cards are in players hands has a fatal flaw: If all cards are in players' hands then they can play any card they want, meaning that unless a card has a special effect of removing more cards from that player's hand then any of them are equally valuable. So if we make a simple modification to the AI we can make the loops much less likely: randomly choose between equally attractive choices.
